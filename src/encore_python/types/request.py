...
class BasicSearch(jsonobject.JsonObject):
    ...
    per_page = jsonobject.IntegerProperty(name="per_page", validators=valid_per_page, required=False, exclude_if_none=True)
    page = jsonobject.IntegerProperty(name="page", validators=positive_num, default=1)
    instrument = jsonobject.StringProperty(name="instrument", choices=instruments, exclude_if_none=False)
    difficulty = jsonobject.StringProperty(name="difficulty", choices=difficulties, exclude_if_none=False)
...
class AdvancedSearch(jsonobject.JsonObject):
    ...
    min_intensity = jsonobject.IntegerProperty(name='minIntensity', validators=positive_num)
    max_intensity = jsonobject.IntegerProperty(name='maxIntensity', validators=positive_num)
    min_average_nps = jsonobject.IntegerProperty(name='minAverageNPS', validators=positive_num)
    max_average_nps = jsonobject.IntegerProperty(name='maxAverageNPS', validators=positive_num)
    ...
    chart_id_after = jsonobject.IntegerProperty(name='chartIdAfter', validators=positive_num, exclude_if_none=True)